...
public class PolicyRestApi extends RestApiTemplate {
  ...
  /*
   * Register a Policy {Token:ownerToken, What:[DS_ID], Whom [UserIDs]}
   */
  public void registerPolicy(RoutingContext rc) {
    ...
    try {
      ...
      if (proceed && body.containsKey("userToken") && body.containsKey("policy")) {
        ...
        for (int i = 0; proceed && i < sensors.size(); i++) {
          ...
          if (proceed && (ownerId_ds == null || !ownerId_ds.equals(ownerId))) {
            owner_verified = false; // if one of the datastream is not owner's datastream we don't proceed ahead,
                                    // May be change this behavior in future
            System.out.println(
                "Either uuid:" + ds_id + ", doesn't exist or you don't have privelegs to assign policy to it.");
        ...
        ArrayList<String> userIdList = new ArrayList<String>();// Creating arraylist of users for which we have to
                                                               // register policy
        ...
        if (proceed && owner_verified) {
          ...
          /*
           * owner is verified. Means for all the DS_ID, ownerId is valid Now: go to the
           * whom construct, and check, if these users exist in Citadel
           */
          ...
          for (int i = 0; proceed && i < users.size(); i++) {
            ...
            // check if userId equals ownerId, this is not allowed???, policy cannot be
            // defined for owner
              ...
              for (int j = 0; j < users.size(); j++) {
                ...
                if (policy.containsKey("where")) {
                  ...
                  if (proceed && where.containsKey("allowedPolygons")) {
                    ...
                    /*
                     * 
                     * Verify that all allowed polygon actually form a continuous polygons Very
                     * Important, else all future queries with this policy will not work
                     */
                  ...
                  if (proceed && where.containsKey("denyPolygons")) {
                    ...
                    /*
                     * 
                     * Verify that all deny polygon actually form a continuous polygons Very
                     * Important, else all future queries with this policy will not work
                     */
                ...
                if (proceed && (allowedPolygons.size() > 0 || denyPolygons.size() > 0 || allowTimes.size() > 0
                    || denyTimes.size() > 0)) {
                ...
                // Inserting the policy below, policy string should be updated to mentioned the
                // correct policy
                if (proceed) {
